  ...
  <script type="text/javascript">
      ...
      var grid = $("#maingrid").ligerGrid({
          columns: 
           ...
           dataAction: 'server',
 		   url: rootPath + 'query/query_getClassList.action?view=WyHT2',
           sortName: 'htid', 
           pageSize: 20, 
           toolbar: {},
      ...
      function toolbarBtnItemClick(item) {
          switch (item.id) {
             case "editht":
                ...
                top.f_addTab(null, '修改合同', '<%=basePath %>adhtDetail.jsp?htid=' + selected.htid );
             	break;
             case "viewht":
             	var selected = grid.getSelected();
                if (!selected) { jQuery.ligerDialog.warn('请选择行!'); return }
                top.f_addTab(null, '查看合同', '<%=basePath %>adhtDetail.jsp?view=1&&htid=' + selected.htid );
             	break;  
             case "returnht":
             	var selected = grid.getSelected();
                if (!selected) { jQuery.ligerDialog.warn('请选择行!'); return }
                jQuery.ligerDialog.confirm('确定退回合同吗?', function (confirm) {
                      if (confirm)
                          f_return();
                  });
             	break;  
             ...
             case "auditht":
            	var selected = grid.getSelected();
                if (!selected) { jQuery.ligerDialog.warn('请选择行!'); return }
                jQuery.ligerDialog.confirm('确定审核通过吗?', function (confirm) {
                      if (confirm)
                          f_auditover();
                  });
                  break; 
      ...
      function f_return() {
          ...
          if (selected) {
           LG.ajax({
                  url: rootPath+'bus/bus_returnWyht.action',
                  loading: '正在退回中...',
                  data: { htid: selected.htid },
                  success: function () {
                      jQuery.ligerDialog.success('退回成功!');
                      f_reload();
                  },
      ...
      function f_auditover() {
          var selected = grid.getSelected();
          if (!selected) { jQuery.ligerDialog.warn('请选择行!'); return }
          if (selected) {
           LG.ajax({
                  url: rootPath+'bus/bus_auditWyht.action',
                  loading: '正在审核中...',
                  data: { htid: selected.htid },
                  success: function () {

  ...
  <script type="text/javascript">
      ...
      var grid = $("#maingrid").ligerGrid({
          columns: 
           ...
           dataAction: 'server',
 		   url: rootPath + 'query/query_getClassList.action?view=WyHT3',
           sortName: 'htid', 
           pageSize: 20, 
           toolbar: {},
      ...
      function toolbarBtnItemClick(item) {
          switch (item.id) {
             ...
             case "viewht":
                ...
                top.f_addTab(null, '查看合同', '<%=basePath %>htDetail.jsp?view=1&&htid=' + selected.htid );
             	break;  
             case "updateht":
             	var selected = grid.getSelected();
                if (!selected) { jQuery.ligerDialog.warn('请选择行!'); return }
                $.ligerDialog.confirm('确实要进行合同变更吗?', function (yes)
                 {
                   	if(yes)f_modifyHT();
                 });
             	break;  
             case "ktcost":
				...
				top.f_addTab(null, '缴纳空调费', '<%=basePath %>jiaonaktlist.jsp?&MenuNo=jiaonakt&ctype=2&htid=' + selected.htid );
             	break;
             case "mangercost":
             	
             	var selected = grid.getSelected();
                if (!selected) { jQuery.ligerDialog.warn('请先选择合同!'); return }
				top.f_addTab(null, '缴纳物业管理费', '<%=basePath %>jiaonawylist.jsp?&MenuNo=jiaonawy&ctype=1&htid=' + selected.htid );
             	break;
          }
      }

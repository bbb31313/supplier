...
//        LG.addSearchButtons(form, grid, container.find("li:eq(0)"), container.find("li:eq(1)"));
    ...
    //附加表单搜索按钮：搜索、高级搜索
    LG.appendDownLoadButtons = function (form, grid)
    {
        if (!form) return;
        form = $(form);
        //搜索按钮 附加到第一个li  高级搜索按钮附加到 第二个li
        ...
        if (btn2Container)
        {
            LG.createButton({
                ...
                click: function ()
                {
                   ...
                   if (rule.rules.length)
                   {
                	   var gridUrl=grid.get('url');
                	   var pos=gridUrl.indexOf("=");
                	   var downView=gridUrl.substr(pos+1);
                	   var sortName=grid.get('sortName');
                	   var expUrl=rootPath +"exp/downLoad?view="+downView+"&sortName="+sortName+"&where="+JSON2.stringify(rule) ;
//                	   LG.showSuccess(sortName);
                	   window.location.href=expUrl;
                   } else
                   {
                	   LG.showError('请至少选择一个筛选项!');
                   }
                }
            });
        }
    ...
    //自定义设置表单底部默认的按钮:保存、取消  by_pdz_20130131
    LG.setCustomFormDefaultBtn = function (cancleCallback, savedCallback)
    {
        //表单底部按钮
        var buttons = [];
        if (cancleCallback)
        ...
        if (!buttons) return;
        var formbar = $("body > div.fr");
        if (formbar.length == 0)
            formbar = $('<div class="fr"><div class="fr-inner"></div></div>').appendTo('body');
    ...
    //设置表单底部按钮:保存、取消                                               --by_pdz_20131119
    LG.setAccountBtn = function (cancleCallback,savedCallback, mangerCallback,ktCallback)
    {
        //表单底部按钮
        var buttons = [];
        if (cancleCallback)
        ...
        if (mangerCallback)
        {
            buttons.push({ text: '管理费', onclick: mangerCallback });
        }
        if (ktCallback)
        {
            buttons.push({ text: '空调费', onclick: ktCallback });
        }
        LG.addFormButtons(buttons);
        
    };
    
    //设置表单底部默认的按钮:查看解决方案、增加解决方案   --by_pdz_20130129
    LG.setSoluDefaultBtn = function (cancleCallback, savedCallback)
    {
        //表单底部按钮
        var buttons = [];
        if (cancleCallback)
        {
            buttons.push({ text: '关闭窗口', onclick: cancleCallback });
        }
        if (savedCallback)
        {
            buttons.push({ text: '增加方案', onclick: savedCallback });
        }
        LG.addFormButtons(buttons);
    };
    ...
    LG.loadToolbar = function (grid, toolbarBtnItemClick)
    {
        ...
        LG.ajax({
            loading: '正在加载工具条中...',
            url:rootPath+'json/json_getToolByMenu.action',
            data: {MenuNo: MenuNo },
            success: function (data)
            {
